...
if __name__ == '__main__':
    ...
    path = '/data/jkim/Rail_road/MxV_Main_Files/TCI_Test/'
    file = 'test08.10.10khz.od4p15__UTC_20220810_214728.772.tdms'  # full set
...
# Convert to Pandas DataFrame
   ...
   # print('df', df.iloc[:5000])
    ...
    #------( Training Data )---------------------------------#
    s_Ch = 0  ; e_Ch=1500 ;  Lb =0  #Normal Position
    Tr_D, Tr_L = Data_parapration(s_Ch, e_Ch, Tr_D, Tr_L, Lb)

    s_Ch = 3100  ; e_Ch=3400 ; Lb =1 #Train Position
    Tr_D, Tr_L = Data_parapration(s_Ch, e_Ch, Tr_D, Tr_L, Lb)

    #-------( Test Data )-----------------------------------#
    s_Ch = 1500  ; e_Ch=3000 ; Lb =0
    NC_Test_D, NC_Test_L = Data_parapration(s_Ch, e_Ch, NC_Test_D, NC_Test_L, Lb)

    s_Ch = 3400  ; e_Ch=3700 ; Lb =1
...
#---Training----------------#
    ...
    model = Sequential()
    model.add(LSTM(65, return_sequences=True, input_shape=(1, X.shape[2])))
    model.add(LSTM(40, return_sequences=False,kernel_initializer=HeUniform()))
    ...
    finalResult = func_f1_Score('<LSTM> Train Position',  Ty_pred_binary,TC_Test_L)
    print(finalResult)
    ...
    finalResult = func_f1_Score('<LSTM> Normal Position', Ny_pred_binary,NC_Test_L)
    print(finalResult)
